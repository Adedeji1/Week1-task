...
const cars = [
  ...
  */
  ...
  const productName = "Ipara"; //strings
  const productPrice = "2000kpi"; //numbers
  const inStock = "true";//booleans
  const features = "maybe"; //object
  
  // Scenario 3: Weather Information
  const temperature = "250c"; //number
  const isSunny = "";//boolean
  
  // Scenario 4: Book Details
  const bookTitle = "Death Note";//object
  const authorDetails = "address,email";
  const isBestseller = "";
  
  // Scenario 5: User Preferences
  const favoriteColors = "black"; //string
  const hasPets = "";
